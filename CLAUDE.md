...
# mostrador web (réplica do firmware, React sem build): http://127.0.0.1:8787/display
...
- **`collector/server.py`** — HTTP `:8787`: `GET /usage` (ESP32), painel em `GET /`, mostrador web (réplica do firmware) em `GET /display`, `GET/POST /api/config`, `POST /api/config/account` e `/api/config/account/delete` (múltiplas contas por provedor). **Sem cache**. Lê `collector/data/config.json` (volume no Docker). Provedores em `collector/providers/`. Painel: `collector/panel.py` + `collector/web/index.html`. Mostrador: `collector/web/display.html` (React via UMD vendorizado em `collector/web/vendor/`, sem build/JSX) — só lê `GET /usage`, mesma UI que o firmware (telas, tema, cor, idioma). Persistência: `collector/store.py`. Docker: `collector/Dockerfile` + `compose.yaml`; `./dev-collector.sh` ou `./dev-collector.sh docker`. Cada provedor em `/usage` é uma **lista de contas** (`docs/CONTRATO_JSON.md`), não um objeto único — suporta N assinaturas do mesmo provedor, cada uma com apelido opcional. Falha em uma conta não pode derrubar as outras; `ok: false` naquela entrada da lista é o contrato.
...
- **`src/ui.cpp`** — controlador das views (Início, detalhe Claude/Cursor/OpenRouter, Info): navegação, toque, redesenho. A pintura de cada view mora em `src/ui_views.cpp`; helpers de formatação/desenho reutilizáveis (barra, botão, texto de data) em `src/ui_format.cpp`. Cada provedor guarda até `MAX_ACCOUNTS` contas (`src/app_state.h`); a Início mostra a que mais precisa de atenção por tipo de provedor, e o detalhe ganha um paginador `‹ i/N ›` quando há mais de uma.
