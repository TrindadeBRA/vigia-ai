...
- **`collector/server.py`** — HTTP `:8787`: `GET /usage` (ESP32), painel em `GET /`, mostrador web (réplica do firmware) em `GET /display`, `GET/POST /api/config`. **Sem cache**. Lê `collector/data/config.json` (volume no Docker). Provedores em `collector/providers/`. Painel: `collector/panel.py` + `collector/web/index.html`. Mostrador: `collector/web/display.html` (React via UMD vendorizado em `collector/web/vendor/`, sem build/JSX) — só lê `GET /usage`, mesma UI que o firmware (telas, tema, cor, idioma). Persistência: `collector/store.py`. Docker: `collector/Dockerfile` + `compose.yaml`; `./dev-collector.sh` ou `./dev-collector.sh docker`. Falha em um provedor não pode derrubar os outros; `ok: false` no objeto daquele provedor é o contrato.
