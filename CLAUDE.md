...
- **`collector/server.py`** — HTTP `:8787`: `GET /usage` (ESP32), painel em `GET /`, `GET/POST /api/config`. **Sem cache**. Lê `collector/data/config.json` (volume no Docker). Provedores em `collector/providers/`. Painel: `collector/panel.py` + `collector/web/index.html`. Persistência: `collector/store.py`. Docker: `collector/Dockerfile` + `compose.yaml`; `./dev-collector.sh` ou `./dev-collector.sh docker`. Falha em um provedor não pode derrubar os outros; `ok: false` no objeto daquele provedor é o contrato.
