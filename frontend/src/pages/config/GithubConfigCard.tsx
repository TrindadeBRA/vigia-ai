...
function RepoRow({ repo, c, onReload }: { repo: GithubRepoConfig; c: ConfigCopy; onReload: () => Promise<void> }) {
    ...
    const repoLabel = repo.label || repo.repo;

    return (
        <li className="flex flex-col gap-2 rounded-[10px] border border-edge bg-canvas px-3 py-2.5">
            <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                    <p className="m-0 overflow-hidden text-ellipsis whitespace-nowrap text-[13.5px] font-[650]">{repoLabel}</p>
            ...
            <ConfirmModal
                ...
                body={c.confirmRemoveBody(repoLabel)}
                confirmLabel={c.remove}
                cancelLabel={c.cancel}
                onCancel={() => setConfirmingRemove(false)}
                onConfirm={() => {
                    setConfirmingRemove(false);
                    void remove.run(async () => { const r = await apiDelete(`/api/github/repos/${repo.id}`); await onReload(); return r; }, { success: c.removed, error: c.offline });
                }}
            />
        </li>
    );
...
function ProfileRow({ profile, c, onReload }: { profile: GithubProfileConfig; c: ConfigCopy; onReload: () => Promise<void> }) {
    ...
    return (
        <li className="flex flex-col gap-2 rounded-[10px] border border-edge bg-canvas px-3 py-2.5">
            <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                    <p className="m-0 overflow-hidden text-ellipsis whitespace-nowrap text-[13.5px] font-[650]">{profileLabel}</p>
            ...
            <ConfirmModal
                open={confirmingRemove}
                title={c.confirmRemoveTitle}
                body={c.confirmRemoveBody(profileLabel)}
                confirmLabel={c.remove}
                cancelLabel={c.cancel}
                onCancel={() => setConfirmingRemove(false)}
                onConfirm={() => {
                    setConfirmingRemove(false);
                    void remove.run(async () => { const r = await apiDelete(`/api/github/profiles/${profile.id}`); await onReload(); return r; }, { success: c.removed, error: c.offline });
                }}
            />
        </li>
    );
