...
export function cursorAllowedSizes(c: CursorAccount | null, metrics?: Metric[]): CardSize[] {
  const count = metrics?.length ?? (c ? getCursorMetrics(c, { cursorModels: "", otherModels: "", ondemand: "", remainingPrefix: "", used: "", cap: "", left: "", bonusPrefix: "", noData: "" } as unknown as T).length : 2);
  if (count >= 3) return ["sm", "sw", "md", "lg", "wl"];
  return ["sm", "sw", "md", "lg"];
}

export const CURSOR_ALLOWED_ALL: CardSize[] = ["sm", "sw", "md", "lg", "wl"];
        ...
        <button type="button" className="flex min-h-0 flex-1 cursor-pointer flex-col justify-center overflow-hidden border-0 bg-transparent p-0 text-left" onClick={onOpen}>
          {single ? (
            <>
              ...
              <div className={cn(num, "mt-1 text-[18px] font-[800] leading-none")}>{single.pct != null ? fmtPct(single.pct) : single.sub || "--"}</div>
              {single.pct != null ? (
