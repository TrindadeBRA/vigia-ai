...
/** Widget de player YouTube Music — espelho do Spotify, mesmo padrão OAuth + polling. */
export function youtubemusicAllowedSizes(): CardSize[] {
  return ["sm", "md", "lg", "free"];
}

export function youtubemusicSizeLabel(size: CardSize, t: T): string {
...
function useYTMusicState(): { state: YTMusicState | null; refresh: () => void } {
  const [state, setState] = useState<YTMusicState | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await fetch("/api/youtubemusic", { cache: "no-store" });
        const data = (await res.json()) as YTMusicState;
  ...
  const refresh = () => {
    fetch("/api/youtubemusic", { cache: "no-store" })
      .then((r) => r.json())
      .then((data) => setState(data as YTMusicState))
      .catch(() => { });
  };
...
function YTMusicLogo({ size = 34 }: { size?: number }) {
  return <img src={PROVIDER_ICON.youtubemusic} alt="" width={size} height={size} className="shrink-0 rounded-full" draggable={false} />;
}
...
function Controls({
  ...
  return (
    <div className={cn("flex items-center", compact ? "gap-1.5" : "gap-2")}>
      <button type="button" className={sideBtn} title={t.ytmusicPrevious} aria-label={t.ytmusicPrevious} disabled={busy} onClick={onPrev}>
        <PrevIcon />
      </button>
      <button type="button" className={mainBtn} title={isPlaying ? t.ytmusicPause : t.ytmusicPlay} aria-label={isPlaying ? t.ytmusicPause : t.ytmusicPlay} disabled={busy} onClick={onPlayPause}>
        {isPlaying ? <PauseIcon /> : <PlayIcon />}
      </button>
      <button type="button" className={sideBtn} title={t.ytmusicNext} aria-label={t.ytmusicNext} disabled={busy} onClick={onNext}>
        <NextIcon />
      </button>
    </div>
  );
...
export function YoutubeMusicBoardCard({ t, size }: { t: T; size: CardSize }) {
  const ns = normalizeSize(size);
  const { state, refresh } = useYTMusicState();
  ...
  function act(kind: "play" | "pause" | "next" | "previous") {
    ...
    void postAction(`/api/youtubemusic/${kind}`).then(() => {
      window.setTimeout(() => { refresh(); setBusy(false); }, AFTER_ACTION_MS);
    });
  }

  if (!state) {
    return <div className="flex h-full min-h-0 w-full items-center justify-center opacity-60"><YTMusicLogo size={28} /></div>;
  }
  ...
  if (!state.configured) {
    return (
      <div className="flex h-full min-h-0 w-full flex-col items-center justify-center gap-2 text-center">
        <YTMusicLogo size={32} />
        <div className={emptyNote}>{t.ytmusicConnectHint}</div>
      </div>
    );
  }
  ...
  if (!track) {
    return (
      <div className="flex h-full min-h-0 w-full flex-col items-center justify-center gap-2 text-center">
        <YTMusicLogo size={isCompact ? 24 : 32} />
        <div className={cardLabel}>{t.ytmusicNothingPlaying}</div>
