...
O firmware **depende** deste formato. Mudança = atualizar este doc, os schemas Zod em `backend/src/schemas.ts` (port de `backend/app/schemas.py`) **e** o parser em `firmware/src/net/parse.cpp`.
