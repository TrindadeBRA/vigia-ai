...
Código vigente: `frontend/src/pages/display/Overview.tsx` (`DndContext`, `DragOverlay`) e `frontend/src/pages/display/BoardTile.tsx` (`useDraggable`/`useDroppable`, handle) e `frontend/src/board.ts` (ordem e `lg`/`sm` persistidos via `/api/board`).
