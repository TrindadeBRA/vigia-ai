...
Nas telas internas (Claude / GPT / Cursor / OpenRouter / DeepSeek / OpenCode Go / OpenCode Zen / fal.ai / Info) e na **Início** (lista ou grade) se os cards não couberem: aparecem **setas** ↑↓ à direita. Toque nelas, deslize vertical, ou serial `u` / `d`.

Hardware: `T_CS` no GPIO 21. Serial: `n` `p` `0`–`7`.

Serial (placa e Wokwi): `n` / `p` Início↔Info, `0` início, `1` Claude, `2` Cursor, `3` OpenRouter, `4` DeepSeek, `5` GPT, `6` OpenCode Go, `7` OpenCode Zen, `8` info, `9` relogio, `l` lista, `g` grade, `t` ciclo de tema, `a` ciclo da cor, `i` ciclo de idioma, `h` ciclo da barra (esq/topo/dir/base), `u`/`d` scroll no detalhe/info/início, `r` refresh, `c` calibrar (só hardware).

A escolha Lista/Grade, o tema, a cor, o idioma e o lado da barra ficam na NVS (namespace `ui`, chaves `home`, `theme`, `accent`, `lang` e `edge`). Início padrão: **grade**. Idioma padrão: **pt-BR**. Barra padrão: **esquerda**. Cor padrão: **vermelho**.
