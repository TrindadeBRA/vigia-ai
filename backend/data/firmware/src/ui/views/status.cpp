...
void paintStatus()
{
  ...
  {
    View cv = g_view;
    if (cv != VIEW_CLAUDE && cv != VIEW_GPT && cv != VIEW_CURSOR && cv != VIEW_OPENROUTER && cv != VIEW_DEEPSEEK && cv != VIEW_OPENCODE && cv != VIEW_FAL && cv != VIEW_BITCOIN && cv != VIEW_ADSENSE && cv != VIEW_CURRENCIES && cv != VIEW_WEATHER)
    {
