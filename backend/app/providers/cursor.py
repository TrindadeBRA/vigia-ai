...
def cursor_fail(msg: str) -> dict[str, Any]:
    return {
        "ok": False,
        "error": msg,
        "percent": None,
...
def _fetch_cursor_with_token(token: str, plan: str | None) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            if parsed.get("ok"):
                return parsed
            return cursor_fail(f"{dash_err}; {parsed.get('error')}")
    except RuntimeError as exc:
        return cursor_fail(f"{dash_err}; fallback: {exc}")
    return cursor_fail(dash_err)


def fetch_cursor_one(token: str, plan: str | None) -> dict[str, Any]:
    token = (token or "").strip()
    if not token:
        return cursor_fail("sem token Cursor")
...
def _fetch_cursor_local(cands: list[tuple[str, str, str | None]]) -> dict[str, Any]:
    ...
    for _source, token, plan in cands:
        ...
        if _is_auth_error(err):
            continue
        return parsed
    return cursor_fail(last_err)


def fetch_cursor_accounts(cfg: dict) -> list[dict[str, Any]]:
    accounts: list[dict[str, Any]] = []
    p = provider_cfg(cfg, "cursor")

    if not p.get("hidden"):
        local_cands = cursor_token_candidates(cfg)
        if local_cands:
